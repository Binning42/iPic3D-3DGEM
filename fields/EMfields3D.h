...
class EMfields3D : public Field{
 ...
 /** initialize the electromagnetic fields with constant values */
  ...
  /** initialize GEM challenge in 3D */
  void initGEM3D(VirtualTopology3D *vct, Grid *grid);
  ...
  /**  Init 3D null point(s) */
  void initNullPoint(VirtualTopology3D *vct, Grid *grid);
  /**  Another 3D null point(s) configuration */
  void initNullPoint1(VirtualTopology3D *vct, Grid *grid);
...
/**  initiliaze EM for GEM challange in 3D */
inline void EMfields3D::initGEM3D(VirtualTopology3D *vct, Grid *grid){
	// perturbation localized in X
	double pertX = 0.4;
	double xpert, ypert, zpert, exp_pert;
	if (restart1 ==0){
		// initialize
		if (vct->getCartesian_rank() ==0){
			cout << "------------------------------------------" << endl;
			cout << "Initialize GEM Challenge in 3D" << endl; 
			cout << "------------------------------------------" << endl;
			cout << "B0x                              = " << B0x << endl;
			cout << "B0y                              = " << B0y << endl;
			cout << "B0z                              = " << B0z << endl;
		...
		for (int i=0; i < nxn; i++)
			for (int j=0; j < nyn; j++)
				for (int k=0; k < nzn; k++){
					...
					for (int is=0; is < ns; is++){
						if (DriftSpecies[is])
							rhons[is][i][j][k] =  (0.01*rhoINIT[is]/FourPI) + (((rhoINIT[is]/(cosh((grid->getYN(i,j,k)-Ly/2)/delta)*cosh((grid->getYN(i,j,k)-Ly/2)/delta))))/FourPI);
						else
							rhons[is][i][j][k] = rhoINIT[is]/FourPI;
					}
					// electric field
					...
					xpert = grid->getXN(i,j,k)- Lx/2;
					ypert = grid->getYN(i,j,k)- Ly/2;
                    			zpert = 0.; //grid->getZN(i,j,k)- Lz/2;
					exp_pert = exp(-(xpert/delta)*(xpert/delta)-(ypert/delta)*(ypert/delta)-(zpert/delta)*(zpert/delta));
					Bxn[i][j][k] +=(B0x*pertX)*exp_pert*(-cos(M_PI*xpert/10.0/delta)*cos(M_PI*ypert/10.0/delta)*2.0*ypert/delta
														 -cos(M_PI*xpert/10.0/delta)*sin(M_PI*ypert/10.0/delta)*M_PI/10.0);
					Byn[i][j][k] +=(B0x*pertX)*exp_pert*(cos(M_PI*xpert/10.0/delta)*cos(M_PI*ypert/10.0/delta)*2.0*xpert/delta
														 +sin(M_PI*xpert/10.0/delta)*cos(M_PI*ypert/10.0/delta)*M_PI/10.0);					
...
*  (c) V. Olshevsky
*/
inline void EMfields3D::initNullPoint(VirtualTopology3D *vct, Grid *grid){
	if (restart1 ==0){
...
*  (c) V. Olshevsky
*/
inline void EMfields3D::initNullPoint1(VirtualTopology3D *vct, Grid *grid){
	if (restart1 ==0){
        ...
        for (int i=0; i < nxn; i++)
          for (int j=0; j < nyn; j++)
            for (int k=0; k < nzn; k++) {
              ...
              Ez[i][j][k] =  0.0;
              // Magnetic field
              Bxn[i][j][k] = B0x*(-cos(2.*M_PI*grid->getXN(i,j,k)/Lx)*sin(2.*M_PI*grid->getYN(i,j,k)/Ly) + pow(cos(2.*M_PI*grid->getYN(i,j,k)/Ly), 2));
              Byn[i][j][k] = B0x*(2.*sin(2.*M_PI*grid->getYN(i,j,k)/Ly)*cos(2.*M_PI*grid->getZN(i,j,k)/Lz) + sin(2.*M_PI*grid->getXN(i,j,k)/Lx)*cos(2.*M_PI*grid->getYN(i,j,k)/Ly));
              Bzn[i][j][k] = B0x*(-2.*sin(2.*M_PI*grid->getZN(i,j,k)/Lz)*cos(2.*M_PI*grid->getYN(i,j,k)/Ly) + pow(cos(2.*M_PI*grid->getYN(i,j,k)/Ly), 2));
        }

        for (int i=0; i <nxc; i++)
		  for (int j=0; j <nyc; j++)
		    for (int k=0; k <nzc; k++) {
			  Bxc[i][j][k] = B0x*(-cos(2.*M_PI*grid->getXC(i,j,k)/Lx)*sin(2.*M_PI*grid->getYC(i,j,k)/Ly) + pow(cos(2.*M_PI*grid->getYC(i,j,k)/Ly), 2));
			  Byc[i][j][k] = B0x*(2.*sin(2.*M_PI*grid->getYC(i,j,k)/Ly)*cos(2.*M_PI*grid->getZC(i,j,k)/Lz) + sin(2.*M_PI*grid->getXC(i,j,k)/Lx)*cos(2.*M_PI*grid->getYC(i,j,k)/Ly));
			  Bzc[i][j][k] = B0x*(-2.*sin(2.*M_PI*grid->getZC(i,j,k)/Lz)*cos(2.*M_PI*grid->getYC(i,j,k)/Ly) + pow(cos(2.*M_PI*grid->getYC(i,j,k)/Ly), 2));
		}
