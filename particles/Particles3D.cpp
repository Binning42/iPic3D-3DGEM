...
/** Add currents for the EMfields3D::initNullPoint initial condition **/
void Particles3D::AddCurrents(Grid* grid,Field* EMf,VirtualTopology3D* vct, double B0, double Lx, double Ly, double Lz){
  double j0 = 2.*M_PI*B0;
  for (register int i=0; i<nop; i++){
		u[i] += j0/q[i]/npcel/invVOL * 2.*cos(2.*M_PI*y[i]/Ly)*cos(2.*M_PI*z[i]/Lz)*(1./Ly + 1./Lz);
		//v[i] += 0.;
		w[i] += j0/q[i]/npcel/invVOL * cos(2.*M_PI*x[i]/Lx)*cos(2.*M_PI*y[i]/Ly)*(1./Lx + 1./Ly);
  }
}
